    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            g = GraphFeatures(
                ...
                as_of=dt.now(tz=timezone.utc),
            )
            rows.append(vectorize(g))
            labels.append(int(row["is_ring_member"]))
    if not rows:
...
def main() -> None:
    ...
    model_dir = STATE_ROOT / "models" / "graph"
    ...
    print(f"trained graph_model version={version}")
    print(f"  n_train={len(train_idx)} n_valid={len(valid_idx)} ring_rate={y.mean():.4f}")
