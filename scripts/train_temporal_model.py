    ...
    with path.open("r", encoding="utf-8") as f:
        for line in f:
            ...
            if not line:
                continue
            row = json.loads(line)
            features.append(vectorize(_parse(row)))
            labels.append(int(row["is_suspicious"]))
    if not features:
...
def main() -> None:
    ...
    model_dir = STATE_ROOT / "models" / "temporal"
    ...
    print(f"trained temporal_model version={version}")
    print(f"  n_train={len(train_idx)} n_valid={len(valid_idx)} suspicious_rate={y.mean():.4f}")
